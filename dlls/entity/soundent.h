...
//MODDD - new
#define bits_SOUND_BAIT     ( 1 << 7 )// bait sound to make monsters in IDLE or ALERT want to investigate.
                                      // Looking at you, chumtoad.
...
public:
	...
	//MODDD - new. If this is not null (given by whoever is making the sound presumably), this method will determine who can listen to the sound.
	// Such as, a chumtoad only wants non-ALIEN_PREY to listen to this sound.
	// If not given (most of the time), assume the monster can listen. Clearly no restriction this way.
	BOOL (* canListenHandle)(CBaseEntity* pOther);

	void setCanListenCheck(  BOOL (* arg_canListenHandle)(CBaseEntity* pOther) );
...
public:
	...
	static CSound*		InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration );
