...
{
	//MODDD - really? bits_SOUND_BAIT has to go in there?
	if ( m_iType & ( bits_SOUND_COMBAT | bits_SOUND_WORLD | bits_SOUND_PLAYER | bits_SOUND_DANGER | bits_SOUND_BAIT ) )
	{
		return TRUE;
	}
...
//MODDD - new
void CSound::setCanListenCheck(  BOOL (* arg_canListenHandle)(CBaseEntity* pOther) ){
...
//MODDD - now returns a pointer to the generated (or desiganted) sound being used.
//void CSoundEnt::InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration )
CSound* CSoundEnt::InsertSound ( int iType, const Vector &vecOrigin, int iVolume, float flDuration )
{
	int iThisSound;
	...
	//MODDD - important to reset this!
	pSoundEnt->m_SoundPool[ iThisSound ].canListenHandle = NULL;

	//MODDD - The as-is script here never considers m_flExpireTime being "SOUND_NEVER_EXPIRE" (-1) as a special case.
	//        The sound would instead disappear instantly.
	//        For instance, say the time (gpGlobals->time) is 24.3 at start.
	//        below would do .m_flExpireTime = 24.3 + (-1) = 23.3.  Expiration checks later would just say, "We're past that, goodbye."
	//        INSTEAD, check for -1 here (or SOUND_NEVER_EXPIRE) and set it HARD to that, not + time.
	//pSoundEnt->m_SoundPool[ iThisSound ].m_flExpireTime = gpGlobals->time + flDuration;
	if(flDuration == SOUND_NEVER_EXPIRE){
		pSoundEnt->m_SoundPool[ iThisSound ].m_flExpireTime = SOUND_NEVER_EXPIRE;
	}else{
		pSoundEnt->m_SoundPool[ iThisSound ].m_flExpireTime = gpGlobals->time + flDuration;
	}
