...
//CBasePlatTrain OLD CLASS LOCATION
...
BOOL CBasePlatTrain::IsWorldAffiliated(){
	return TRUE;
}
...
BOOL CFuncPlat::IsWorldAffiliated(){
	return TRUE;
}
...
//CFuncPlatRot OLD CLASS LOCATION
...
public:
	...
	virtual BOOL IsWorldAffiliated();
...
BOOL CFuncTrain::IsWorldAffiliated(){
	return TRUE;
}
...
BOOL CFuncTrackTrain::IsWorldAffiliated(){
	return TRUE;
}
...
void CFuncTrackTrain::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( useType != USE_SET )
	{
		...
		if ( pev->speed == 0 )
		{
			...
			//Update this. If the player makes a change this gets updated instantly to be restored.
			this->m_oldSpeed = pev->speed;
...
void CFuncTrackTrain::Next( void )
{
	...
	if ( !pev->speed )
	{
		...
		//MODDD - This turns off think, in case we have another Think (Next method here) call coming up. This avoids playing this
		//        sound a second time on coming back when pev->nextthink is reached (time-wise). Or clearly we weren't planning
		//        on resetting pev->nextthink to Think again by stopping here. Either way this is fine or a tiny bit redundant, no problem.
		//pev->nextthink = 0;
		//MODDD - bad idea!!! Turning off think here causes the cart to stall when the player uses controls while going up / down a 
		//        track platform, AKA func_trackchange.
		//        Don't really have time to look into that more right now, if it works fine as it is it stays.
...
//		m_ppath = CPathTrack::Instance(FIND_ENTITY_BY_TARGETNAME( NULL, STRING(pev->target) ));
	...
	if(EASY_CVAR_GET_DEBUGONLY(trackTrainPrintouts)){
		if(pnext != NULL){
			...
			easyForcePrintLine("TRAIN: Current path:%s, ID:%d. LookAhead: NONE?", m_ppath->pev->targetname!=NULL?STRING(m_ppath->pev->targetname):"NULL", m_ppath->PathTrackID);
		}
	}
	...
	if ( pnext )
	{
		if ( pnext != m_ppath )
		{
			...
			//!!! DEBUG FETAURE
			const char* messageCheat = STRING(pFire->pev->message);
...
BOOL CFuncTrainControls::IsWorldAffiliated(){
	return TRUE;
}
...
//CFuncTrackChange class OLD LOCATION. See trains.h now.
...
TYPEDESCRIPTION	CFuncTrackChange::m_SaveData[] = 
{
	...
	//MODDD - to remember.
	DEFINE_FIELD( CFuncTrackChange, FuncTrackChangeID, FIELD_INTEGER ),
...
//MODDD
int CFuncTrackChange::FuncTrackChangeIDLatest = 0;

const char* TRAIN_CODE_STR[] = { "TRAIN_SAFE", "TRAIN_BLOCKING", "TRAIN_FOLLOWING"};


void CFuncTrackChange::Spawn( void )
{
	//My ID is the next one in line.
	FuncTrackChangeID = FuncTrackChangeIDLatest;
...
BOOL CFuncTrackChange::IsWorldAffiliated(){
	return TRUE;
}
...
//MODDD NOTE - printouts changed to refer to trackchangePrintouts instead for conveniently showing only these instead of a lot of other printouts associated with "developer" on.
...
void CFuncTrackChange::ReportGeneric(){
	CBaseEntity::ReportGeneric();


	easyForcePrintLine("CFuncTrackChange Specific:");

	if(m_trackTop != NULL){
		easyForcePrintLine("-trackTop:  pathTrackID:%d Enabled:%d", m_trackTop->PathTrackID, (m_trackTop->pev->spawnflags&SF_PATH_DISABLED)==0);
	}else{
		easyForcePrintLine("-trackTop:  NULL");
	}

	if(m_trackBottom != NULL){
		easyForcePrintLine("-trackBottom:  pathTrackID:%d Enabled:%d", m_trackBottom->PathTrackID, (m_trackBottom->pev->spawnflags&SF_PATH_DISABLED)==0);
	}else{
		easyForcePrintLine("-trackBottom:  NULL");
	}
...
//		UpdateTrain();
	...
	//is that okay?
	if(SF_TRACK_ACTIVATETRAIN){
		//This sets the train to half speed on returning to use. as in tick 2/4.

		//m_train->pev->speed = m_train->m_speed * 0.5;
		m_train->pev->speed = m_train->m_oldSpeed;

		m_train->Next();	
		m_train->pev->iuser1 = 1;
	}
...
public:
	...
	//MODDD
	Vector			BodyTargetMod( const Vector &posSrc ) { return pev->origin; }
...
BOOL CGunTarget::isOrganic(void){
	return FALSE;
}
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CGunTarget)
{
	GENERATE_TRACEATTACK_PARENT_CALL(CBaseMonster);
}

GENERATE_TAKEDAMAGE_IMPLEMENTATION(CGunTarget)
{
	if ( pev->health > 0 )
	{
		pev->health -= flDamage;
...
BOOL CGunTarget::IsWorldAffiliated(){
	return TRUE;
}
