...
#if EXTRA_NAMES > 0
	LINK_ENTITY_TO_CLASS( bullchicken, CBullsquid );
	LINK_ENTITY_TO_CLASS( bullsquid, CBullsquid );

	#if EXTRA_NAMES == 2
		LINK_ENTITY_TO_CLASS( monster_bullsquid, CBullsquid );
		LINK_ENTITY_TO_CLASS( monster_bull, CBullsquid );
...
Task_t tlSquidEat[] =
{
	...
	{ TASK_WAIT_FOR_MOVEMENT_RANGE,		(float)56				},
...
Schedule_t *CBullsquid::GetSchedule( void )
{
	...
	case MONSTERSTATE_COMBAT:
		{
// dead enemy
			//easyForcePrintLine("SOOOOO DOES THIS MAKE SENSE TO YEW? %d", HasConditions( bits_COND_CAN_RANGE_ATTACK1 ) );
			...
			//easyForcePrintLine("SOOOOO DOES THIS MAKE SENSE TO AYYYYYYY? %d", HasConditions( bits_COND_CAN_RANGE_ATTACK1 ) );
...
Schedule_t* CBullsquid::GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_RANGE_ATTACK1:
		...
		//MODDD - we must be allowed to do this!  Don't rapid-fire...

		//MODDD - can disable the bullsquid's range attack.  If so, chase the enemy instead to try Melee.
