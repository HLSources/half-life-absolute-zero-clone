...
#if EXTRA_NAMES > 0
	LINK_ENTITY_TO_CLASS( osprey, COsprey );
...
void COsprey::Spawn( void )
{
	...
	pev->classname = MAKE_STRING("monster_osprey");
...
CBaseMonster *COsprey::MakeGrunt( Vector vecSrc )
{
	...
	for (int i = 0; i < m_iUnits; i++)
	{
		if (m_hGrunt[i] == NULL || !m_hGrunt[i]->IsAlive())
		{
			...
			pent = CREATE_NAMED_ENTITY( MAKE_STRING( "monster_human_grunt" ));
			if ( FNullEnt( pent ) )
			{
...
void COsprey::FlyThink( void )
{
	...
	//MODDD - if we can't find a goal node, do NOT proceed or else there will be a crash.
	//Putting nullchecks for m_pGoalEnt later wouldn't make much difference, it would not end up flying without
	//the map providing some route.
	if(m_pGoalEnt == NULL){
		//Stay grounded!
		return;
	}
...
void COsprey::DyingThink( void )
{
	...
	if (m_startTime > gpGlobals->time )
	{
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSpot );
			...
			// flags
			//MODDD - osprey gibs now have the smoketrail too.
