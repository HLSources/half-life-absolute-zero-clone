...
//MODDD NOTE - all cases of "SET_MODEL" have been replaced with "setModel", a new method for all CBaseEntity's, so that anything that treats setting the model as an event may do something on this occurence.
...
//MODDD - if these are going to be the same just put them together...
...
void CMultiManager::ManagerThink ( void )
{
	...
	{
		//MODDD NOTE - use the "FireTargetsTest" version and edit that one to give whatever extra printouts you want.
...
void CBaseTrigger::HurtTouch ( CBaseEntity *pOther )
{
	...
	CBaseMonster* tempMon = pOther->GetMonsterPointer();
	const char* clsnm;
	if(tempMon != NULL){
		clsnm = tempMon->getClassname();

		if(tempMon->IsPlayer() == TRUE){
			tempMon = NULL;
			//Drop the monster (player) because we don't want to spam printous about the player.
		}
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH A. %s:%d spawnflags? targetOnce:%d startOff:%d noClients:%d clientOnlyFire:%d clientOnlyTouch:%d", clsnm, tempMon->monsterID,
		(pev->spawnflags & SF_TRIGGER_HURT_TARGETONCE)!=0,
		(pev->spawnflags & SF_TRIGGER_HURT_START_OFF)!=0,
		(pev->spawnflags & SF_TRIGGER_HURT_NO_CLIENTS)!=0,
		(pev->spawnflags & SF_TRIGGER_HURT_CLIENTONLYFIRE)!=0,
		(pev->spawnflags & SF_TRIGGER_HURT_CLIENTONLYTOUCH)!=0
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH B. %s:%d", clsnm, tempMon->monsterID);
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH C. %s:%d", clsnm, tempMon->monsterID);
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH D. %s:%d", clsnm, tempMon->monsterID);
	...
	else	// Original code -- single player
	{
		...
		//monster_scientist:6 time:130.42 dmgtime:130.70 pain_finished:130.20 

		if(tempMon)easyForcePrintLine("Trigger HURTTOUCH E. %s:%d", clsnm, tempMon->monsterID);
		if ( pev->dmgtime > gpGlobals->time && gpGlobals->time != pev->pain_finished )
		{// too early to hurt again, and not same frame with a different entity
			if(tempMon)easyForcePrintLine("Trigger HURTTOUCH E - EARLY FAIL! %s:%d time:%.2f dmgtime:%.2f pain_finished:%.2f ", clsnm, tempMon->monsterID, gpGlobals->time, pev->dmgtime, pev->pain_finished);
			return;
		}
	}
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH F. %s:%d", clsnm, tempMon->monsterID);
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH G. %s:%d", clsnm, tempMon->monsterID);
	...
	if(tempMon)easyForcePrintLine("Trigger HURTTOUCH H. %s:%d", clsnm, tempMon->monsterID);
	
	if ( pev->target )
	{
		if(tempMon)easyForcePrintLine("Trigger HURTTOUCH I. %s:%d", clsnm, tempMon->monsterID);
		// trigger has a target it wants to fire. 
		if ( pev->spawnflags & SF_TRIGGER_HURT_CLIENTONLYFIRE )
		{
			if(tempMon)easyForcePrintLine("Trigger HURTTOUCH J. %s:%d", clsnm, tempMon->monsterID);
			// if the toucher isn't a client, don't fire the target!
			if ( !pOther->IsPlayer() )
			{
				if(tempMon)easyForcePrintLine("Trigger HURTTOUCH K. %s:%d", clsnm, tempMon->monsterID);
				return;
			}
		}

		if(tempMon)easyForcePrintLine("Trigger HURTTOUCH L. %s:%d", clsnm, tempMon->monsterID);
...
void CBaseTrigger::HurtTouch ( CBaseEntity *pOther )
{
	...
	*/
	...
	//No removal of player features besides bare essentials to move at all show why the trigger does not do continual damage
	//to monsters standing still in it. It is, zany.
	//Fudging a monster's velocity or position... probably velocity... can let the touch method work, but its still weird why that would be necessary.
	/*
	//if(!pOther->IsPlayer())
	{
	...
	if ( pEdicttt ){
		for (i = 1; i < gpGlobals->maxEntities; i++, pEdicttt++ ){
			...
			if(  (tempmon = pEntityyy->GetMonsterPointer()) != NULL){
				...
				if(tempmon->wasAttached < 2){
					tempmon->wasAttached = FALSE;
				}
			}
...
//					ALERT( at_console, "Trying %s\n", STRING(pEntity->pev->classname) );
					...
					if ( !(caps & FCAP_DONT_SAVE) )
					{
						...
						if ( flags )
						{
							...
							pEntity->alreadySaved = TRUE;


							CBaseMonster* monsterTest;

							monsterTest = pEntity->GetMonsterPointer();


							//This fixes a little bug: monsters in the middle of dying between a level transition reset their death anim, potentially letting the player reset the death anim
							//over and over again by triggering the level transition before the death animation finishes.
							//This forces the death animation to warp to the end (last frame) if an entity in a dying animation is transferred to the new level.
							if(monsterTest != NULL && monsterTest->pev->deadflag == DEAD_DYING){
								//force them dead, and at the last frame.
								//TODO: overrride for foes with varried death behavior like the gargantua? Anything special need to be done before skipping to that last frame?
								//The gargantua could also override DeathAnimationEnd to do something that must be guaranteed done by or at the end of the death anim too.
								
								//just in case... is that fine to call Start() too?
	...
	if ( pEdicttt ){
		for (i = 1; i < gpGlobals->maxEntities; i++, pEdicttt++ ){
			...
			if(  (tempmon = pEntityyy->GetMonsterPointer()) != NULL && tempmon->m_pCine != NULL){
				...
				if(tempmon->alreadySaved == FALSE && tempmon->m_pCine->alreadySaved == TRUE){
					//host ent not saved, but its m_pCine is?  Mark its m_pCine as "wasAttached" hard   then to remember.

					//easyForcePrintLine("AW man IM SO HAPPY   %s %s", tempmon->getClassname(), tempmon->m_pCine->getClassname());

					tempmon->m_pCine->wasAttached = 2;
				}
...
void CTriggerCamera::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	//MODDD - note.  AMAZING!  This is used right before the end-credits for whatever reason, take a look with "playerBrightLight".
	//easyPrintLine("?????? %d", FBitSet (pev->spawnflags, SF_CAMERA_PLAYER_TAKECONTROL ) );
