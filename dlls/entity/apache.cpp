...
void CApache::DyingThink( void )
{
	...
	if (m_flNextRocket > gpGlobals->time )
	{
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSpot );
			...
			// flags
			//MODDD - apache gibs now have the smoketrail too.
...
GENERATE_TAKEDAMAGE_IMPLEMENTATION(CApache)
{
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CApache)
{
	...
	else
	{
		...
		// (found commented out as-is, I think)
...
void CApacheHVR::Precache( void )
{
	...
	global_useSentenceSave = TRUE;
	PRECACHE_SOUND ("weapons/rocket1.wav", TRUE); //player.
